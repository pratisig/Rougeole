...
Version corrigée selon les 11 points demandés
============================================================
"""
...
# CONFIG STREAMLIT
st.set_page_config(page_title="Surveillance Rougeole Multi-pays", layout="wide", page_icon="🦠", initial_sidebar_state="expanded")

st.markdown("""<style>.metric-card{background-color:#f0f2f6;padding:15px;border-radius:10px;box-shadow:2px 2px 5px rgba(0,0,0,0.1)}.high-risk{background-color:#ffebee;color:#c62828;font-weight:bold;padding:5px;border-radius:3px}.medium-risk{background-color:#fff3e0;color:#ef6c00;padding:5px;border-radius:3px}.low-risk{background-color:#e8f5e9;color:#2e7d32;padding:5px;border-radius:3px}.stButton>button{width:100%}h1{color:#d32f2f}.info-box{background-color:#e3f2fd;padding:10px;border-left:4px solid #2196f3;margin:10px 0}</style>""", unsafe_allow_html=True)
...
PAYS_ISO3_MAP = {"Niger": "ner", "Burkina Faso": "bfa", "Mali": "mli", "Mauritanie": "mrt"}
...
option_aire = st.sidebar.radio("Source des données géographiques", ["Fichier local (ao_hlthArea.zip)", "Upload personnalisé"], key='option_aire')

pays_selectionne = None
iso3_pays = None

if option_aire == "Fichier local (ao_hlthArea.zip)":
    pays_selectionne = st.sidebar.selectbox("🌍 Sélectionner le pays", list(PAYS_ISO3_MAP.keys()), key='pays_select')
...
if option_aire == "Upload personnalisé":
    upload_file = st.sidebar.file_uploader("Charger un fichier géographique", type=["shp", "geojson", "zip"], help="Format: Shapefile ou GeoJSON avec colonnes 'iso3' et 'health_area'")

st.sidebar.subheader("📊 Données Épidémiologiques")
option_linelist = st.sidebar.radio("Source des cas de rougeole", ["Upload CSV", "Données fictives (test)"], key='option_linelist')

linelist_file = None
if option_linelist == "Upload CSV":
    linelist_file = st.sidebar.file_uploader("Linelists rougeole (CSV)", type=["csv"], help="Format: health_area, Semaine_Epi, Cas_Total OU Date_Debut_Eruption, Aire_Sante...")
...
pred_mois = st.sidebar.slider("Période de prédiction (mois)", min_value=1, max_value=12, value=3, help="Nombre de mois à prédire après la dernière semaine de données")
...
with st.sidebar.expander("Configurer les seuils", expanded=False):
    seuil_baisse = st.slider("Seuil de baisse significative (%)", min_value=10, max_value=90, value=75, step=5, help="Afficher les aires avec baisse ≥ X% par rapport à la moyenne")
    seuil_hausse = st.slider("Seuil de hausse significative (%)", min_value=10, max_value=200, value=50, step=10, help="Afficher les aires avec hausse ≥ X% par rapport à la moyenne")
    seuil_alerte_epidemique = st.number_input("Seuil d'alerte épidémique (cas/semaine)", min_value=1, max_value=100, value=5, help="Nombre de cas par semaine déclenchant une alerte")
...
def generate_dummy_linelists(_sa_gdf, n=500, start=None, end=None):
    ...
    dates = pd.to_datetime(start) + pd.to_timedelta(np.random.exponential(scale=delta_days/3, size=n).clip(0, delta_days).astype(int), unit="D")
    df = pd.DataFrame({"ID_Cas": range(1, n+1), "Date_Debut_Eruption": dates, "Date_Notification": dates + pd.to_timedelta(np.random.poisson(3, n), unit="D"), "Aire_Sante": np.random.choice(_sa_gdf["health_area"].unique(), n), "Age_Mois": np.random.gamma(shape=2, scale=30, size=n).clip(6, 180).astype(int), "Statut_Vaccinal": np.random.choice(["Oui", "Non"], n, p=[0.55, 0.45]), "Sexe": np.random.choice(["M", "F"], n), "Issue": np.random.choice(["Guéri", "Décédé", "Inconnu"], n, p=[0.92, 0.03, 0.05])})
    return df
...
with st.spinner("📥 Chargement données de cas..."):
    if option_linelist == "Upload CSV" and linelist_file:
        try:
            ...
            if "Semaine_Epi" in df_raw.columns and "Cas_Total" in df_raw.columns:
                ...
                for _, row in df_raw.iterrows():
                    ...
                    for i in range(cas_total):
                        expanded_rows.append({"ID_Cas": len(expanded_rows) + 1, "Date_Debut_Eruption": base_date + timedelta(days=np.random.randint(0, 7)), "Date_Notification": base_date + timedelta(days=np.random.randint(0, 10)), "Aire_Sante": aire, "Age_Mois": 0, "Statut_Vaccinal": "Inconnu", "Sexe": "Inconnu", "Issue": "Inconnu"})
            ...
            st.stop()
    elif option_linelist == "Données fictives (test)":
...
def worldpop_children_stats(_sa_gdf, use_gee):
    if not use_gee:
        ...
        return pd.DataFrame({"health_area": _sa_gdf["health_area"], "Pop_Totale": [np.nan] * len(_sa_gdf), "Pop_Moins_15": [np.nan] * len(_sa_gdf)})
    try:
        features = []
        ...
        for feat in stats_info['features']:
            ...
            data_list.append({"health_area": props.get("health_area", ""), "Pop_Moins_15": int(pop_sum), "Pop_Totale": int(pop_sum * 2.2)})
        return pd.DataFrame(data_list)
    except Exception as e:
        st.sidebar.warning(f"⚠️ WorldPop: {str(e)[:50]}")
        return pd.DataFrame({"health_area": _sa_gdf["health_area"], "Pop_Totale": [np.nan] * len(_sa_gdf), "Pop_Moins_15": [np.nan] * len(_sa_gdf)})

with st.spinner("🔄 WorldPop..."):
...
def urban_classification(_sa_gdf, use_gee):
    ...
    try:
        ...
        st.sidebar.warning(f"⚠️ GHSL: {str(e)[:50]}")
        return pd.DataFrame({"health_area": _sa_gdf["health_area"], "Urbanisation": [np.nan] * len(_sa_gdf)})

with st.spinner("🔄 GHSL..."):
...
def fetch_climate_nasa_power(_sa_gdf, start_date, end_date):
    ...
    for idx, row in _sa_gdf.iterrows():
        ...
        params = {"parameters": "T2M,PRECTOTCORR,RH2M", "community": "AG", "longitude": lon, "latitude": lat, "start": start_date.strftime("%Y%m%d"), "end": end_date.strftime("%Y%m%d"), "format": "JSON"}
        try:
            ...
            if "properties" in j and "parameter" in j["properties"]:
                ...
                data_list.append({"health_area": row["health_area"], "Temperature_Moy": temp_mean, "Humidite_Moy": rh_mean, "Saison_Seche_Humidite": rh_mean * 0.7 if not np.isnan(rh_mean) else np.nan})
            else:
...
sa_gdf_enrichi["Densite_Moins_15"] = sa_gdf_enrichi["Pop_Moins_15"] / sa_gdf_enrichi["Superficie_km2"].replace(0, np.nan)
sa_gdf_enrichi = sa_gdf_enrichi.replace([np.inf, -np.inf], np.nan)
st.sidebar.success("✓ Enrichissement terminé")
# PARTIE 2/4 - KPIs, CARTE ET ANALYSES
...
cases_by_area = df.groupby("Aire_Sante").agg({"ID_Cas": "count", "Statut_Vaccinal": lambda x: (x == "Non").mean() * 100, "Age_Mois": "mean"}).reset_index()
cases_by_area.columns = ["Aire_Sante", "Cas_Observes", "Taux_Non_Vaccines", "Age_Moyen"]
sa_gdf_with_cases = sa_gdf_enrichi.merge(cases_by_area, left_on="health_area", right_on="Aire_Sante", how="left")
...
sa_gdf_with_cases["Taux_Attaque_10000"] = (sa_gdf_with_cases["Cas_Observes"] / sa_gdf_with_cases["Pop_Moins_15"].replace(0, np.nan) * 10000).replace([np.inf, -np.inf], np.nan)
...
if max_cases > 0:
    colormap = cm.LinearColormap(colors=['#e8f5e9', '#81c784', '#ffeb3b', '#ff9800', '#f44336', '#b71c1c'], vmin=0, vmax=max_cases, caption="Nombre de cas observés")
    colormap.add_to(m)

for idx, row in sa_gdf_with_cases.iterrows():
    aire_name = row['health_area']
    cas_obs = int(row.get('Cas_Observes', 0))
    pop_moins_15 = row.get('Pop_Moins_15', np.nan)
    taux_attaque = row.get('Taux_Attaque_10000', np.nan)
    urbanisation = row.get('Urbanisation', 'N/A')
    popup_html = f"""<div style="font-family: Arial; width: 350px;"><h3 style="margin-bottom: 10px; color: #1976d2; border-bottom: 2px solid #1976d2;">{aire_name}</h3><div style="background-color: #f5f5f5; padding: 10px; margin: 10px 0; border-radius: 5px;"><h4 style="margin: 0; color: #d32f2f;">📊 Situation Épidémiologique</h4><table style="width: 100%; margin-top: 5px;"><tr><td><b>Cas observés:</b></td><td style="text-align: right;"><b style="font-size: 18px; color: #d32f2f;">{cas_obs}</b></td></tr><tr><td>Population <15 ans:</td><td style="text-align: right;">{f"{int(pop_moins_15):,}" if not np.isnan(pop_moins_15) else "N/A"}</td></tr><tr><td>Taux d'attaque:</td><td style="text-align: right;">{f"{taux_attaque:.1f}/10K" if not np.isnan(taux_attaque) else "N/A"}</td></tr><tr><td>Type habitat:</td><td style="text-align: right;"><b>{urbanisation if pd.notna(urbanisation) else "N/A"}</b></td></tr></table></div></div>"""
    ...
    if row['Cas_Observes'] >= seuil_alerte_epidemique:
        ...
        line_color = 'black'
        line_weight = 1
    folium.GeoJson(row['geometry'], style_function=lambda x, color=fill_color, weight=line_weight, border=line_color: {'fillColor': color, 'color': border, 'weight': weight, 'fillOpacity': 0.7}, tooltip=folium.Tooltip(f"<b>{aire_name}</b><br>{cas_obs} cas", sticky=True), popup=folium.Popup(popup_html, max_width=400)).add_to(m)
    if cas_obs > 0:
        folium.Marker(location=[row.geometry.centroid.y, row.geometry.centroid.x], icon=folium.DivIcon(html=f"""<div style="font-size: 9pt; color: black; font-weight: bold; background-color: rgba(255,255,255,0.7); padding: 2px; border-radius: 3px; white-space: nowrap;">{aire_name}</div>""")).add_to(m)

heat_data = [[row.geometry.centroid.y, row.geometry.centroid.x, row['Cas_Observes']] for idx, row in sa_gdf_with_cases.iterrows() if row['Cas_Observes'] > 0]
if heat_data:
    HeatMap(heat_data, radius=20, blur=25, max_zoom=13, gradient={0.0: 'blue', 0.5: 'yellow', 1.0: 'red'}).add_to(m)

legend_html = f'''<div style="position: fixed; bottom: 50px; left: 50px; width: 250px; background-color: white; border: 2px solid grey; z-index:9999; font-size:14px; padding: 10px; border-radius: 5px;"><p style="margin: 0; font-weight: bold;">📊 Légende</p><p style="margin: 5px 0;"><span style="background-color: #e8f5e9; padding: 2px 8px;">Faible</span> 0-{max_cases//3:.0f} cas</p><p style="margin: 5px 0;"><span style="background-color: #ffeb3b; padding: 2px 8px;">Moyen</span> {max_cases//3:.0f}-{2*max_cases//3:.0f} cas</p><p style="margin: 5px 0;"><span style="background-color: #f44336; padding: 2px 8px; color: white;">Élevé</span> >{2*max_cases//3:.0f} cas</p><p style="margin: 5px 0; padding-top: 5px; border-top: 1px solid #ccc;"><b>Seuil alerte:</b> {seuil_alerte_epidemique} cas/sem</p></div>'''
...
fig_epi = go.Figure()
fig_epi.add_trace(go.Scatter(x=weekly_cases['Semaine_Label'], y=weekly_cases['Cas'], mode='lines+markers', name='Cas observés', line=dict(color='#d32f2f', width=3), marker=dict(size=6), hovertemplate='<b>%{x}</b><br>Cas: %{y}<extra></extra>'))
...
if len(weekly_cases) > 5:
    ...
    fig_epi.add_trace(go.Scatter(x=weekly_cases['Semaine_Label'], y=tendance, mode='lines', name='Tendance', line=dict(color='#1976d2', width=2, dash='dash'), hovertemplate='<b>%{x}</b><br>Tendance: %{y:.1f}<extra></extra>'))
fig_epi.add_hline(y=seuil_alerte_epidemique, line_dash="dot", line_color="orange", annotation_text=f"Seuil d'alerte ({seuil_alerte_epidemique} cas/sem)", annotation_position="right")
fig_epi.update_layout(title="Courbe épidémique par semaines épidémiologiques", xaxis_title="Semaine épidémiologique", yaxis_title="Nombre de cas", hovermode='x unified', height=400)
st.plotly_chart(fig_epi, use_container_width=True)

col1, col2, col3 = st.columns(3)
with col1:
    semaine_max = weekly_cases.loc[weekly_cases['Cas'].idxmax()]
    st.metric("🔴 Semaine avec pic maximal", semaine_max['Semaine_Label'], f"{int(semaine_max['Cas'])} cas")
with col2:
...
df["Tranche_Age"] = pd.cut(df["Age_Mois"], bins=[0, 12, 60, 120, 180], labels=["0-1 an", "1-5 ans", "5-10 ans", "10-15 ans"])
age_stats = df.groupby("Tranche_Age").agg({"ID_Cas": "count", "Statut_Vaccinal": lambda x: (x == "Non").mean() * 100}).reset_index()
age_stats.columns = ["Tranche_Age", "Nombre_Cas", "Pct_Non_Vaccines"]
col1, col2 = st.columns(2)
with col1:
    fig_age = px.bar(age_stats, x="Tranche_Age", y="Nombre_Cas", title="Cas par tranche d'âge", color="Nombre_Cas", color_continuous_scale="Reds", text="Nombre_Cas")
    fig_age.update_traces(textposition='outside')
    st.plotly_chart(fig_age, use_container_width=True)
with col2:
    fig_vacc_age = px.bar(age_stats, x="Tranche_Age", y="Pct_Non_Vaccines", title="% non vaccinés par âge", color="Pct_Non_Vaccines", color_continuous_scale="Oranges", text="Pct_Non_Vaccines")
...
st.info("""**Nowcasting (Prévision immédiate):** Technique d'ajustement permettant d'estimer le nombre réel de cas en tenant compte des délais de notification. Les cas récents sont souvent sous-estimés car certaines notifications n'ont pas encore été enregistrées. Cette méthode applique un facteur de correction basé sur le délai moyen observé.""")
...
with col1:
    st.metric("Délai moyen de notification", f"{delai_moyen:.1f} jours", help="Temps moyen entre l'apparition des symptômes et la notification officielle")
with col2:
    st.metric("Délai médian", f"{delai_median:.0f} jours", help="Valeur centrale des délais observés")
with col3:
    st.metric("Écart-type", f"{delai_std:.1f} jours", help="Variabilité des délais de notification")
with col4:
    ...
    st.metric(f"Cas corrigés ({derniere_semaine_label})", cas_corriges, delta=f"+{cas_corriges - cas_derniere_semaine} (ajustement)", help="Estimation du nombre réel de cas après correction des délais")
fig_delai = px.histogram(df, x="Delai_Notification", nbins=20, title="Distribution des délais de notification", labels={"Delai_Notification": "Délai (jours)", "count": "Nombre de cas"}, color_discrete_sequence=['#d32f2f'])
...
# PARTIE 3/4 - MODÉLISATION PRÉDICTIVE

st.header("🔮 Modélisation Prédictive par Semaines Épidémiologiques")
st.markdown(f"""<div class="info-box"><b>Configuration de la prédiction:</b><br>• Dernière semaine de données: <b>S{derniere_semaine_epi} ({derniere_annee})</b><br>• Période de prédiction: <b>{pred_mois} mois ({n_weeks_pred} semaines)</b><br>• Semaines prédites: <b>S{derniere_semaine_epi+1} à S{min(derniere_semaine_epi+n_weeks_pred, 52)}</b><br>• Seuils configurés: Baisse ≥{seuil_baisse}%, Hausse ≥{seuil_hausse}%</div>""", unsafe_allow_html=True)
...
if st.session_state.prediction_lancee:
    with st.spinner("🤖 Entraînement et prédiction..."):
        weekly_features = df.groupby(["Aire_Sante", "Annee", "Semaine_Epi"]).agg(Cas_Observes=("ID_Cas", "count"), Non_Vaccines=("Statut_Vaccinal", lambda x: (x == "Non").mean() * 100), Age_Moyen=("Age_Mois", "mean")).reset_index()
        weekly_features['Semaine_Label'] = weekly_features['Annee'].astype(str) + '-S' + weekly_features['Semaine_Epi'].astype(str).str.zfill(2)
        weekly_features = weekly_features.merge(sa_gdf_enrichi[["health_area", "Pop_Totale", "Pop_Moins_15", "Densite_Moins_15", "Urbanisation", "Temperature_Moy", "Humidite_Moy", "Saison_Seche_Humidite"]], left_on="Aire_Sante", right_on="health_area", how="left")
        ...
        feature_cols = ["Cas_Observes", "Non_Vaccines", "Age_Moyen", "Pop_Totale", "Pop_Moins_15", "Densite_Moins_15", "Urban_Encoded", "Temperature_Moy", "Saison_Seche_Humidite", "Cas_Lag_1", "Cas_Lag_2", "Cas_Lag_3", "Cas_Lag_4", "Semaine_Epi"]
        weekly_features_clean = weekly_features.dropna(subset=feature_cols)
        if len(weekly_features_clean) < 20:
            st.warning("⚠️ Données insuffisantes (minimum 20 observations)")
            st.stop()
        ...
        models = {"GradientBoosting": GradientBoostingRegressor(n_estimators=300, learning_rate=0.05, max_depth=5, min_samples_split=4, random_state=42), "RandomForest": RandomForestRegressor(n_estimators=200, max_depth=10, min_samples_split=4, random_state=42)}
        best_model = None
        best_score = -np.inf
        best_name = ""
        col1, col2 = st.columns(2)
        for name, model in models.items():
            ...
            with col1 if name == "GradientBoosting" else col2:
                st.metric(f"📊 {name}", f"R² = {score:.3f}", f"CV: {cv_mean:.3f} (±{cv_scores.std():.3f})")
        
        st.success(f"✓ Meilleur modèle: **{best_name}** (R² CV = {best_score:.3f})")
        feature_importance = pd.DataFrame({"Variable": feature_cols, "Importance": best_model.feature_importances_}).sort_values("Importance", ascending=False)
        with st.expander("📊 Importance des variables", expanded=False):
            fig_imp = px.bar(feature_importance.head(10), x="Importance", y="Variable", orientation="h", title="Top 10 variables", color="Importance", color_continuous_scale="Viridis")
        ...
        for aire in weekly_features["Aire_Sante"].unique():
            ...
            for i in range(1, n_weeks_pred + 1):
                ...
                future_row = {"Aire_Sante": aire, "Annee": nouvelle_annee, "Semaine_Epi": nouvelle_semaine_epi, "Semaine_Label": f"{nouvelle_annee}-S{str(nouvelle_semaine_epi).zfill(2)}", "Non_Vaccines": last_obs["Non_Vaccines"], "Age_Moyen": last_obs["Age_Moyen"], "Pop_Totale": last_obs["Pop_Totale"], "Pop_Moins_15": last_obs["Pop_Moins_15"], "Densite_Moins_15": last_obs["Densite_Moins_15"], "Urban_Encoded": last_obs["Urban_Encoded"], "Temperature_Moy": last_obs["Temperature_Moy"], "Saison_Seche_Humidite": last_obs["Saison_Seche_Humidite"]}
                if i == 1:
        ...
        st.success(f"✓ {len(future_df)} prédictions ({len(future_df['Aire_Sante'].unique())} aires × {n_weeks_pred} semaines)")
        ...
        risk_df = future_df.groupby("Aire_Sante").agg(Cas_Predits_Total=("Predicted_Cases", "sum"), Cas_Predits_Max=("Predicted_Cases", "max"), Cas_Predits_Moyen=("Predicted_Cases", "mean"), Semaine_Pic=("Predicted_Cases", lambda x: future_df.loc[x.idxmax(), "Semaine_Label"] if len(x) > 0 else "N/A")).reset_index()
        risk_df = risk_df.merge(moyenne_historique, on="Aire_Sante", how="left")
        risk_df["Variation_Pct"] = ((risk_df["Cas_Predits_Moyen"] - risk_df["Moyenne_Historique"]) / risk_df["Moyenne_Historique"].replace(0, 1)) * 100
        risk_df["Categorie_Variation"] = pd.cut(risk_df["Variation_Pct"], bins=[-np.inf, -seuil_baisse, -10, 10, seuil_hausse, np.inf], labels=["Forte baisse", "Baisse modérée", "Stable", "Hausse modérée", "Forte hausse"])
        ...
        fig_var_dist = px.pie(risk_df, names="Categorie_Variation", title="Distribution des tendances prédites", color="Categorie_Variation", color_discrete_map={"Forte baisse": "#2e7d32", "Baisse modérée": "#81c784", "Stable": "#9e9e9e", "Hausse modérée": "#ff9800", "Forte hausse": "#d32f2f"})
        st.plotly_chart(fig_var_dist, use_container_width=True)
# PARTIE 4/4 - HEATMAP, CARTE PRÉDICTIONS, TABLEAUX ET EXPORTS

        st.subheader("🔥 Heatmap Temporelle des Prédictions")
        heatmap_data = future_df.pivot(index="Aire_Sante", columns="Semaine_Label", values="Predicted_Cases").fillna(0)
        ...
        fig_heatmap = px.imshow(heatmap_data_top, labels=dict(x="Semaine Épidémiologique", y="Aire de Santé", color="Cas prédits"), x=heatmap_data_top.columns, y=heatmap_data_top.index, title=f"Évolution prédite - Top 20 aires (10 hausses + 10 baisses)", color_continuous_scale=["#e8f5e9", "#81c784", "#ffeb3b", "#ff9800", "#f44336", "#b71c1c"], aspect="auto")
        fig_heatmap.update_xaxes(side="bottom", tickangle=-45, tickmode='array', tickvals=list(range(0, len(heatmap_data_top.columns), max(1, len(heatmap_data_top.columns)//10))), ticktext=[heatmap_data_top.columns[i] for i in range(0, len(heatmap_data_top.columns), max(1, len(heatmap_data_top.columns)//10))])
        fig_heatmap.update_layout(height=600)
        st.plotly_chart(fig_heatmap, use_container_width=True)
        
        tab1, tab2, tab3 = st.tabs([f"🔴 Hausse ≥{seuil_hausse}%", f"🟢 Baisse ≥{seuil_baisse}%", "📊 Toutes les aires"])
        with tab1:
            ...
            if len(hausse_df) > 0:
                ...
                st.dataframe(hausse_df[["Aire_Sante", "Moyenne_Historique", "Cas_Predits_Moyen", "Variation_Pct", "Semaine_Pic", "Cas_Predits_Max"]].style.apply(highlight_hausse, axis=1).format({"Moyenne_Historique": "{:.1f}", "Cas_Predits_Moyen": "{:.1f}", "Variation_Pct": "{:+.1f}%", "Cas_Predits_Max": "{:.0f}"}), use_container_width=True)
        ...
        with tab2:
            ...
            if len(baisse_df) > 0:
                ...
                st.dataframe(baisse_df[["Aire_Sante", "Moyenne_Historique", "Cas_Predits_Moyen", "Variation_Pct", "Semaine_Pic", "Cas_Predits_Max"]].style.apply(highlight_baisse, axis=1).format({"Moyenne_Historique": "{:.1f}", "Cas_Predits_Moyen": "{:.1f}", "Variation_Pct": "{:+.1f}%", "Cas_Predits_Max": "{:.0f}"}), use_container_width=True)
        ...
        with tab3:
            ...
            def highlight_all(row):
                if row["Variation_Pct"] >= seuil_hausse:
                    ...
                    return [""] * len(row)
            st.dataframe(risk_df[["Aire_Sante", "Moyenne_Historique", "Cas_Predits_Moyen", "Variation_Pct", "Categorie_Variation", "Semaine_Pic", "Cas_Predits_Max"]].style.apply(highlight_all, axis=1).format({"Moyenne_Historique": "{:.1f}", "Cas_Predits_Moyen": "{:.1f}", "Variation_Pct": "{:+.1f}%", "Cas_Predits_Max": "{:.0f}"}), use_container_width=True, height=400)
        
        st.subheader("🗺️ Carte des Prédictions")
        sa_gdf_pred = sa_gdf_enrichi.merge(risk_df, left_on="health_area", right_on="Aire_Sante", how="left")
        ...
        colormap_pred = cm.LinearColormap(colors=['#2e7d32', '#81c784', '#e0e0e0', '#ff9800', '#d32f2f'], vmin=-max_var, vmax=max_var, caption="Variation (%) par rapport à la moyenne")
        colormap_pred.add_to(m_pred)
        for idx, row in sa_gdf_pred.iterrows():
            popup_html = f"""<div style="font-family: Arial; width: 360px;"><h3 style="color: #1976d2; border-bottom: 2px solid #1976d2;">{row['health_area']}</h3><div style="background-color: {'#ffebee' if row.get('Variation_Pct', 0) >= seuil_hausse else '#e8f5e9' if row.get('Variation_Pct', 0) <= -seuil_baisse else '#f5f5f5'}; padding: 10px; margin: 10px 0; border-radius: 5px;"><h4 style="margin: 0;">🔮 Prédictions</h4><table style="width: 100%; margin-top: 5px;"><tr><td><b>Moyenne historique:</b></td><td style="text-align: right;">{row.get('Moyenne_Historique', 0):.1f} cas/sem</td></tr><tr><td><b>Moyenne prédite:</b></td><td style="text-align: right;">{row.get('Cas_Predits_Moyen', 0):.1f} cas/sem</td></tr><tr><td><b>Variation:</b></td><td style="text-align: right; font-size: 18px; color: {'#d32f2f' if row.get('Variation_Pct', 0) >= seuil_hausse else '#2e7d32' if row.get('Variation_Pct', 0) <= -seuil_baisse else '#000'};"><b>{row.get('Variation_Pct', 0):+.1f}%</b></td></tr><tr><td>Tendance:</td><td style="text-align: right;"><b>{row.get('Categorie_Variation', 'N/A')}</b></td></tr><tr><td>Semaine du pic:</td><td style="text-align: right;">{row.get('Semaine_Pic', 'N/A')}</td></tr><tr><td>Pic maximal:</td><td style="text-align: right;">{int(row.get('Cas_Predits_Max', 0))} cas</td></tr></table></div></div>"""
            fill_color = colormap_pred(row.get('Variation_Pct', 0)) if pd.notna(row.get('Variation_Pct')) else '#e0e0e0'
            folium.GeoJson(row['geometry'], style_function=lambda x, color=fill_color: {'fillColor': color, 'color': 'black', 'weight': 1, 'fillOpacity': 0.7}, tooltip=folium.Tooltip(f"<b>{row['health_area']}</b><br>Variation: {row.get('Variation_Pct', 0):+.1f}%", sticky=True), popup=folium.Popup(popup_html, max_width=400)).add_to(m_pred)
        ...
        with col1:
            ...
            st.download_button(label="📥 Synthèse prédictions (CSV)", data=csv_pred, file_name=f"predictions_synthese_{pays_selectionne if pays_selectionne else 'pays'}_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
        with col2:
            csv_detail = future_df.to_csv(index=False).encode('utf-8')
            st.download_button(label="📅 Détail par semaine (CSV)", data=csv_detail, file_name=f"predictions_detail_{pays_selectionne if pays_selectionne else 'pays'}_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
        with col3:
            geojson_str = sa_gdf_pred.to_json()
            st.download_button(label="🗺️ Carte prédictions (GeoJSON)", data=geojson_str, file_name=f"carte_predictions_{pays_selectionne if pays_selectionne else 'pays'}_{datetime.now().strftime('%Y%m%d')}.geojson", mime="application/json")
        output = BytesIO()
        ...
        st.download_button(label="📊 Rapport complet (Excel)", data=output.getvalue(), file_name=f"rapport_complet_{pays_selectionne if pays_selectionne else 'pays'}_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        ...
        st.markdown("---")
        st.caption(f"""**Méthodologie de prédiction:** Modèle: {best_name} | Score R² (validation croisée): {best_score:.3f} | Variables: Historique (4 semaines), démographie, urbanisation, climat, vaccination | Période de prédiction: S{derniere_semaine_epi+1} à S{min(derniere_semaine_epi+n_weeks_pred, 52)} ({n_weeks_pred} semaines) | Seuils: Baisse ≥{seuil_baisse}%, Hausse ≥{seuil_hausse}%, Alerte épidémique ≥{seuil_alerte_epidemique} cas/sem""")
else:
    st.info("👆 Cliquez sur le bouton ci-dessus pour lancer la modélisation prédictive")
    st.markdown("""### 📚 Ce que vous obtiendrez : ✅ **Prédictions par semaines épidémiologiques** (S1 à S52) ✅ **Identification des aires à risque** selon vos seuils personnalisés ✅ **Heatmap temporelle** (évolution semaine par semaine) ✅ **Cartes interactives** avec prédictions ✅ **Export multi-formats** (CSV, Excel, GeoJSON) ✅ **Recommandations opérationnelles** basées sur les résultats""")

st.markdown("---")
st.caption(f"""Dashboard de Surveillance Rougeole - Version 2.0 (Semaines Épidémiologiques) | Généré le {datetime.now().strftime('%d/%m/%Y à %H:%M')} | Pays: {pays_selectionne if pays_selectionne else 'Non spécifié'} | Période: {start_date} - {end_date} | Nombre d'aires: {len(sa_gdf)} | Cas analysés: {len(df):,}""")
