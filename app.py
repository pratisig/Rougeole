...
# ============================================================
# PARTIE 5/6 - MODÉLISATION PRÉDICTIVE
# ============================================================
...
if st.session_state.prediction_lancee:
    ...
    with st.spinner("🤖 Préparation des données et entraînement..."):
        ...
        if donnees_dispo["Climat"]:
            feature_cols.extend(["Temperature_Moy", "Humidite_Moy", "Saison_Seche_Humidite"])
            st.info("✅ Données climatiques intégrées au modèle")
                ...
                if donnees_dispo["Climat"]:
                    if future_climate is not None:
                        ...
                        if not climate_aire.empty:
                            future_row.update({
                                "Temperature_Moy": climate_aire.iloc[0]["Temperature_Moy"],
                                "Humidite_Moy": climate_aire.iloc[0]["Humidite_Moy"],
                                "Saison_Seche_Humidite": climate_aire.iloc[0]["Saison_Seche_Humidite"]
                            })
                        else:
                            future_row.update({
                                "Temperature_Moy": last_obs["Temperature_Moy"],
                                "Humidite_Moy": last_obs["Humidite_Moy"],
...
Méthodologie de prédiction: Modèle: {modele_choisi} | Score R² (validation croisée): {cv_mean:.3f} (±{cv_std:.3f}) | Variables: {len(feature_cols)} features (historique 4 semaines, démographie, urbanisation, climat, vaccination) | Période: S{derniere_semaine_epi+1} à S{min(derniere_semaine_epi+n_weeks_pred, 52)} ({n_weeks_pred} semaines) | Seuils: Baisse ≥{seuil_baisse}%, Hausse ≥{seuil_hausse}%, Alerte ≥{seuil_alerte_epidemique} cas/sem """)
...
✅ Prédictions par semaines épidémiologiques (S1 à S52) ✅ Identification des aires à risque selon vos seuils personnalisés ✅ Heatmap temporelle (évolution semaine par semaine) ✅ Cartes interactives avec prédictions ✅ Export multi-formats (CSV, Excel, GeoJSON) ✅ Recommandations opérationnelles basées sur les résultats ✅ Intégration automatique des données disponibles (climat, vaccination, démographie) """)
...
POIDS_MANUELS = {
    ...
    "Pop_Totale": 0.15,
    "Urban_Encoded": 0.08,
    "Temperature_Moy": 0.01,
    "Humidite_Moy": 0.01,
    # Autres variables...
}

# Créer un vecteur de poids pour chaque feature
sample_weights = []
for col in feature_cols:
    weight = POIDS_MANUELS.get(col, 0.01)  # 0.01 par défaut
...
# Appliquer lors de l'entraînement
if modele_choisi == "GradientBoosting (Recommandé)":
    model = GradientBoostingRegressor(
        n_estimators=300,
        learning_rate=0.05,
    ...
    # Pour GradientBoosting, pas de sample_weight direct sur les features
    # Mais on peut pondérer les observations
    model.fit(X_train, y_train)

OPTION 2 : Pondération personnalisée avec transformation manuelle
python

# À ajouter après la création de X (avant le scaling)

# Définir les poids que vous voulez
FEATURE_WEIGHTS = {
    "Cas_Observes": 0.40,
    "Non_Vaccines": 0.35,
    "Pop_Totale": 0.15,
    "Urban_Encoded": 0.08,
    "Coef_Climatique": 0.02  # Si vous créez cette variable composite
}

# Créer une variable climatique composite si données disponibles
if donnees_dispo["Climat"]:
    weekly_features["Coef_Climatique"] = (
        weekly_features["Temperature_Moy"] * 0.5 + 
        weekly_features["Humidite_Moy"] * 0.3 +
        weekly_features["Saison_Seche_Humidite"] * 0.2
    )
    # Normaliser entre 0 et 1
    from sklearn.preprocessing import MinMaxScaler
    scaler_climat = MinMaxScaler()
    weekly_features["Coef_Climatique"] = scaler_climat.fit_transform(
        weekly_features[["Coef_Climatique"]]
    )

# Appliquer les poids manuellement aux colonnes
X_weighted = X.copy()
for i, col in enumerate(feature_cols):
    if col in FEATURE_WEIGHTS:
        X_weighted[:, i] = X_weighted[:, i] * FEATURE_WEIGHTS[col]
    else:
        # Poids par défaut pour les variables non spécifiées
        X_weighted[:, i] = X_weighted[:, i] * 0.01

# Utiliser X_weighted au lieu de X pour l'entraînement
X_train, X_test, y_train, y_test = train_test_split(
    X_weighted, y, test_size=0.2, random_state=42
)

OPTION 3 (RECOMMANDÉE) : Système hybride avec importance ajustable
python

# Ajouter dans la sidebar
st.sidebar.subheader("⚖️ Pondération des Variables")
mode_importance = st.sidebar.radio(
    "Mode d'importance",
    ["Automatique (ML)", "Manuel (Expert)"],
    help="Automatique : calculé par le modèle | Manuel : poids définis par l'expert"
)

poids_manuels = {}
if mode_importance == "Manuel (Expert)":
    with st.sidebar.expander("Configurer les poids", expanded=True):
        st.info("Les poids doivent totaliser 1.0")
        
        poids_manuels["Cas_Observes"] = st.slider(
            "Historique des cas", 0.0, 1.0, 0.40, 0.05
        )
        poids_manuels["Non_Vaccines"] = st.slider(
            "Taux non-vaccinés", 0.0, 1.0, 0.35, 0.05
        )
        poids_manuels["Pop_Totale"] = st.slider(
            "Population", 0.0, 1.0, 0.15, 0.05
        )
        poids_manuels["Urban_Encoded"] = st.slider(
            "Urbanisation", 0.0, 1.0, 0.08, 0.05
        )
        poids_manuels["Climat"] = st.slider(
            "Facteurs climatiques", 0.0, 1.0, 0.02, 0.01
        )
        
        total_poids = sum(poids_manuels.values())
        if abs(total_poids - 1.0) > 0.01:
            st.warning(f"⚠️ Total des poids = {total_poids:.2f} (devrait être 1.0)")

# Dans la section d'entraînement
if mode_importance == "Manuel (Expert)":
    # Appliquer les poids manuels
    X_weighted = X.copy()
    for i, col in enumerate(feature_cols):
        if col in poids_manuels:
            X_weighted.iloc[:, i] = X_weighted.iloc[:, i] * poids_manuels[col]
        elif "Climat" in poids_manuels and col in ["Temperature_Moy", "Humidite_Moy", "Saison_Seche_Humidite"]:
            X_weighted.iloc[:, i] = X_weighted.iloc[:, i] * (poids_manuels["Climat"] / 3)
...
Je vous suggère d'utiliser l'OPTION 3 (hybride) car :

    ✅ Flexibilité : L'utilisateur peut choisir automatique ou manuel
    ✅ Traçabilité : On voit clairement quels poids sont utilisés
    ✅ Expertise : Permet d'intégrer les connaissances épidémiologiques
    ✅ Validation : On peut comparer les performances des deux approches

Avantages de votre approche manuelle :

    Basée sur l'expertise épidémiologique
    Reproductible
    Explicable aux décideurs

Avantages de l'approche automatique actuelle :

    S'adapte aux données réelles
    Peut découvrir des relations inattendues
    Optimisée pour la prédiction

Voulez-vous que je vous fournisse le code complet avec l'option hybride intégrée ?

oui
10:36 AM

Voici le code complet avec le système hybride d'importance des variables intégré :
python
...
# ========== NOUVEAU : SYSTÈME HYBRIDE D'IMPORTANCE DES VARIABLES ==========
...
if st.session_state.prediction_lancee:
    ...
    with st.spinner("🤖 Préparation des données et entraînement..."):
        ...
        if donnees_dispo["Climat"]:
            # Normaliser d'abord chaque variable climatique entre 0 et 1
            scaler_climat = MinMaxScaler()
            ...
            # Créer le coefficient climatique composite
            weekly_features["Coef_Climatique"] = (
        ...
        if mode_importance == "👨‍⚕️ Manuel (Expert)":
            ...
            for group_name, weight in poids_normalises.items():
                if group_name in feature_groups:
                    ...
                    if len(cols_in_group) > 0:
                        # Distribuer le poids équitablement entre les colonnes du groupe
            ...
            for col in feature_cols:
                if col not in column_weights:
                    column_weights[col] = 0.01  # Poids minimal
            ...
            for i in range(1, n_weeks_pred + 1):
                ...
                if donnees_dispo["Climat"]:
                    if future_climate is not None:
                        climate_aire = future_climate[future_climate["health_area"] == aire]
                        if not climate_aire.empty:
                            # Calculer le coefficient climatique pour les prédictions futures
